...
describe("Prisma 6 Effect Generator", () => {
  ...
  describe("Batch operations", () => {
    ...
    it.effect("updateManyAndReturn - should support select", () =>
      Effect.gen(function* () {
        ...
        yield* prisma.user.createMany({
          data: [{ email: `${prefix}-1@example.com`, name: "Original" }],
        });
  ...
  describe("Pagination", () => {
    ...
    it.effect("should support reverse pagination with negative take", () =>
      Effect.gen(function* () {
        ...
        const beforeCursorIds = beforeCursor
          .map((u) => u.id)
          .sort((a, b) => a - b);
  ...
  describe("Transactions", () => {
    ...
    it.effect(
      "nested tx: deeply nested (3 levels) - all rollback on innermost fail",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "nested tx: catch inner failure - inner data PERSISTS (no savepoints)",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
              ...
              const innerResult = yield* prisma
                .$transaction(
                  Effect.gen(function* () {
                    yield* prisma.user.create({
                      data: {
                        email: `${prefix}-inner@example.com`,
                        name: "Inner",
                      },
                    });
                    yield* Effect.fail("Inner failure");
    ...
    it.effect(
      "nested tx: function with its own $transaction called from outer tx",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
          ...
          const outerResult = yield* prisma
            .$transaction(
              Effect.gen(function* () {
                ...
                const outerUser = yield* prisma.user.create({
                  data: {
                    email: `${prefix}-outer@example.com`,
                    name: "Outer User",
                  },
                });
    ...
    it.effect(
      "nested tx: composable service functions that use transactions",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "nested tx: service function failure rolls back entire composed tx",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should support transaction with multiple operations and return value",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should rollback all operations on late failure in transaction",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should rollback on Prisma OrThrow error same as Effect.fail",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
          ...
          const effectFailProgram = prisma.$transaction(
            Effect.gen(function* () {
              yield* prisma.user.create({
                data: {
                  email: `${prefix}-effect@example.com`,
                  name: "Effect User",
                },
              });
          ...
          const prismaErrorProgram = prisma.$transaction(
            Effect.gen(function* () {
              yield* prisma.user.create({
                data: {
                  email: `${prefix}-prisma@example.com`,
                  name: "Prisma User",
                },
              });
    ...
    it.effect(
      "should rollback on update non-existent record in transaction",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should NOT rollback when catching update error on non-existent",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should allow partial recovery with catchTag in transaction",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
          ...
          const result = yield* prisma.$transaction(
            Effect.gen(function* () {
              ...
              const user2 = yield* prisma.user
                .findUniqueOrThrow({
                  where: { email: `${prefix}-ghost@example.com` },
                })
                .pipe(
                  Effect.catchTag("PrismaRecordNotFoundError", () =>
    ...
    it.effect(
      "should preserve Ref modifications even on transaction failure",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
          ...
          const program = prisma.$transaction(
            Effect.gen(function* () {
              ...
              yield* Ref.update(operationLog, (log) => [
                ...log,
                "user-created",
              ]);
          ...
          expect(finalLog).toEqual(["tx-start", "user-created", "tx-failed"]);
    ...
    it.effect(
      "should allow modifying service state from inside transaction",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
  ...
  describe("Error handling", () => {
    ...
    it.effect(
      "should return PrismaRecordNotFoundError on OrThrow methods",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should return PrismaForeignKeyConstraintError on invalid FK",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should return PrismaRecordNotFoundError on update non-existent",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "should return PrismaRecordNotFoundError on delete non-existent",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
  ...
  describe("Raw queries", () => {
    ...
    it.effect("$queryRawUnsafe should execute unsafe SELECT", () =>
      Effect.gen(function* () {
        ...
        yield* prisma.user.create({
          data: { email, name: "Unsafe Query Test" },
        });
  ...
  describe("Service layer composition with transactions", () => {
    ...
    it.effect("single service layer wrapping Prisma - tx rollback works", () =>
      Effect.gen(function* () {
        ...
        const RepoLayer = Layer.merge(Prisma.layer(), UserRepo.Default);
    ...
    it.effect(
      "three-level service composition - tx rollback works through all layers",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
          ...
          class AuthorService extends Effect.Service<AuthorService>()(
            "AuthorService",
            {
              effect: Effect.gen(function* () {
                const users = yield* UserRepo;
                return {
                  ...
                  getProfile: (id: number) => users.findById(id),
                };
              }),
            }
          ) {}

          class ContentService extends Effect.Service<ContentService>()(
            "ContentService",
            {
              effect: Effect.gen(function* () {
                const posts = yield* PostRepo;
          ...
          const Level2 = Layer.merge(
            AuthorService.Default,
            ContentService.Default
          ).pipe(Layer.provide(Level1));
          ...
          const successProgram = Effect.gen(function* () {
            ...
            return yield* onboarding.onboardAuthor(
              `${prefix}-success@example.com`,
              "Success"
            );
          });

          const result = yield* successProgram.pipe(
          ...
          const failProgram = Effect.gen(function* () {
            const onboarding = yield* OnboardingService;
            yield* onboarding.onboardAndFail(
          ...
          yield* prisma.post.deleteMany({
            where: { authorId: result.author.id },
          });
    ...
    it.effect(
      "service method that returns Effect can be composed in transactions",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
    ...
    it.effect(
      "complex: 4-level deep service with stored effects and delayed execution",
      () =>
        Effect.gen(function* () {
          const prisma = yield* Prisma;
          ...
          class AuthorDomain extends Effect.Service<AuthorDomain>()(
            "AuthorDomain",
            {
              effect: Effect.gen(function* () {
                const users = yield* UserRepo;
                ...
                return {
                  ...
                  registerAuthor: (email: string, name: string) =>
                    createUser(email, name),
                  addPost: (title: string, authorId: number) =>
                    posts.create(title, authorId),
                };
              }),
            }
          ) {}
            ...
            effect: Effect.gen(function* () {
              ...
              return {
                onboardWithPost: (
                  ...
                  Effect.gen(function* () {
                    const author = yield* authorDomain.registerAuthor(
                      email,
                      name
                    );
                    const post = yield* authorDomain.addPost(
                      postTitle,
                      author.id
                    );
                    return { author, post };
          ...
          class Orchestrator extends Effect.Service<Orchestrator>()(
            "Orchestrator",
            {
              effect: Effect.gen(function* () {
                const db = yield* Prisma;
                ...
                return {
                  safeOnboard: (
                    ...
                    db.$transaction(
                      blogApp.onboardWithPost(email, name, postTitle)
                    ),
                };
              }),
            }
          ...
          // Cleanup successful user
          yield* prisma.post.deleteMany({
            where: { authorId: okResult.author.id },
          });
